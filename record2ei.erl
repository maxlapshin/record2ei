...
main([Path, OutDir]) ->
  ...
  Filename = filename:basename(Path, ".erl"),
  ParserPath = filename:join(OutDir, Filename ++ "_ei.c"),
  HeaderName = Filename ++ "_ei.h",
  HeaderPath = filename:join(OutDir, HeaderName),

  #gen{headers = Headers, parser = Parser} = generate(Rows, #gen{parser = [
    "#include \"", HeaderName,"\"\n\n"
  ]}),


  % io:format("// HEADERS\n~s~n~n", [Headers]),
  filelib:ensure_dir(HeaderPath),
  filelib:ensure_dir(ParserPath),
  file:write_file(HeaderPath, Headers),
  file:write_file(ParserPath, Parser),
  % io:format("// PARSER\n~s~n~n", [Parser]),
...
generate_type({record,Record}, Desc, #gen{headers = Headers, parser = Parser, structs = Structs} = Gen) ->
  ...
  lists:map(fun
    ...
    ({Int, Field, _}) when Int == non_neg_integer orelse Int == integer -> ["  ei_x_encode_long(x, r.",Field,");\n"];
